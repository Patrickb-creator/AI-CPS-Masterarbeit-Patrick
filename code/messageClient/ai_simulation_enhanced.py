...
__thesis_author__ = 'Patrick Beyer, pbeyer@uni-potsdam.de'
...
def task_worker():
    ...
    while True:
        ...
        if receiver == client_id:
            ...
            client.publish(
                MQTT_Topic_Results,
                f"{client_id}: scenario={scenario}, knowledge_base={knowledge_base}, activation_base={activation_base}, code_base={code_base}, learning_base={learning_base} - Task executed."
                )       
...
if __name__ == '__main__':
    ...
    client.publish(MQTT_Topic_Execute, 'Hi there! My name is '+ client_id +' and I have subscribed to topic '+ MQTT_Topic_Execute+'. RPi=NO') #RPi=NO if not a Pi
