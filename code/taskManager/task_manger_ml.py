...
def on_message(client, userdata, msg):
    ...
    # ────────────── finish/# (wenn alle Aufgaben einer Runde erledigt) ──────────────
        ...
        if clients_with_tasks == finisher_counter:
            ...
            log_directory_power = os.path.join(script_dir, "power_logs_ml_2_final")
            os.makedirs(log_directory_power, exist_ok=True)
            timestamp = time.strftime("%Y-%m-%d %H-%M-%S")
            file_path = os.path.join(log_directory_power, f"{timestamp}_power-log-ml_2__final{task_num}.csv")
