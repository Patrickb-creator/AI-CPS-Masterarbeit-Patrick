...
def create_task(scenario, host, MQTT_topic):
   # function creates the specific MQTT message based on the scenario
...
def task_generator(number_of_tasks, MQTT_topic, client, host):
    scenarios, task_counts = generate_exact_tasknumber(number_of_tasks)
    
    tasks = []
    scenario_count = {scenario: 0 for scenario in scenarios}

    for scenario, count in zip(scenarios, task_counts):
        for _ in range(count):
            task = create_task(scenario, host, "mqttTester")
            tasks.append(task)
            scenario_count[scenario] += 1  # increment counter for selected scenario
    ...
    print(f"{number_of_tasks} tasks were saved in {output_file}.")
